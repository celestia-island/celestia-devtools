...
Two emit modes:
  ...
  - ``tag(source, line)`` — prepend SOURCE column to a line streamed from a
    subprocess (ANSI escapes stripped, then re-aligned to the columnar layout)

This is the canonical copy.  Repos that still ship a vendored
``scripts/utils/logger.py`` can either ``from celestia_devtools import logger``
or keep their local copy as a fallback; both are byte-identical.
"""
